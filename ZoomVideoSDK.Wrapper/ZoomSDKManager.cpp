...
namespace ZoomVideoSDKWrapper {
    ...
    bool ZoomSDKManager::StopVideo()
    {
        ...
        {
            OnSessionStatusChanged(SessionStatus::Error, String::Format("Failed to stop video: {0}", (int)ret));
            return false;
        }
    }
    ...
    bool ZoomSDKManager::IsUserVideoOn(void* user)
    {
        ...
        try {
            ...
            // Check video status as described in blog post: pUserInfo->getVideoPipe()->getVideoStatus().isOn
            ZoomVideoSDKVideoStatus videoStatus = videoPipe->getVideoStatus();
            return videoStatus.isOn;
        }
        catch (...) {
    ...
    bool ZoomSDKManager::IsCurrentUserVideoOn()
    {
        if (!m_bInitialized || !m_pVideoSDK)
            return false;
        ...
        try {
            ...
            // Use the same logic as IsUserVideoOn for current user
            return IsUserVideoOn(currentUser);
        }
        catch (...) {
            return false;
        }
    ...
    bool ZoomSDKManager::GetVideoStatus(void* user)
    {
        // This method is the same as IsUserVideoOn for consistency
        return IsUserVideoOn(user);
    }
